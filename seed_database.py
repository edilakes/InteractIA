...
def seed_fundamental_skills():
    """
    Inserta las habilidades fundamentales y de comunicación en la base de datos
    si no existen. Estas son cruciales para el funcionamiento del agente.
    """
    print("--- Iniciando siembra de habilidades fundamentales ---")
    
    # Primero, verifica si la conexión a la KB es posible.
    # La propia clase KnowledgeBase imprime errores si no puede conectar.
    kb = KnowledgeBase()
    if not kb.client:
        print("\n(-) ERROR CRÍTICO: No se pudo establecer conexión con MongoDB.")
        print("Asegúrate de que tu archivo .env tiene la variable MONGO_URI configurada correctamente.")
        print("Siembra abortada.")
        return

    # Definición del documento de habilidades fundamentales
    ...
    # Usar la función aprender_habilidad para insertar o actualizar el documento
    try:
        print(f"Intentando sembrar el documento '{habilidades_fundamentales['nombre_recurso']}'...")
        ...
        print("\n(+) ¡Éxito! La base de datos ha sido inicializada con las habilidades fundamentales.")
        print("Puedes iniciar la aplicación de nuevo.")

    except Exception as e:
        print(f"\n(-) Ocurrió un error durante la siembra: {e}")
        print("Por favor, revisa el error y tu configuración de base de datos.")

if __name__ == "__main__":
    seed_fundamental_skills()
